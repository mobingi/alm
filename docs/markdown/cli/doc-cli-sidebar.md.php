<!-- Sidebar is not parsed thru script to give you more control over style-->

<ul class="nav sidenav dropable sticky">
    <li class="active"><a href="#overview">Overview</a></li>
    <li><a href="#global-flags">Global flags</a></li>
    <li><a href="#login">login</a></li>
    <li>
        <a href="#stack-list">stack</a>
        <ul>
            <li><a href="#stack-list">list</a></li>
            <li><a href="#stack-describe">describe</a></li>
            <li><a href="#stack-create">create</a></li>
            <li><a href="#stack-update">update</a></li>
            <li><a href="#stack-delete">delete</a></li>
            <li><a href="#stack-ssh">ssh</a></li>
            <li><a href="#stack-exec">exec</a></li>
            <li><a href="#stack-pem">pem</a></li>
        </ul>
    </li>
    <li>
        <a href="#template-versions">template</a>
        <ul>
            <li><a href="#template-versions">versions</a></li>
            <li><a href="#template-compare">compare</a></li>
        </ul>
    </li>
    <li>
        <a href="#rbac-describe">rbac</a>
        <ul>
            <li><a href="#rbac-describe">describe</a></li>
            <li><a href="#rbac-sample">sample</a></li>
            <li><a href="#rbac-create">create</a></li>
            <li><a href="#rbac-attach">attach</a></li>
            <li><a href="#rbac-delete">delete</a></li>
        </ul>
    </li>
    <li>
        <a href="#svrconf-show">svrconf</a>
        <ul>
            <li><a href="#svrconf-show">show</a></li>
            <li><a href="#svrconf-update">update</a></li>
        </ul>
    </li>
    <li>
        <a href="#creds-view">creds</a>
        <ul>
            <li><a href="#creds-view">list</a></li>
        </ul>
    </li>
    <li>
        <a href="#registry-list-catalog">registry</a>
        <ul>
            <li><a href="#registry-list-catalog">catalog</a></li>
            <li><a href="#registry-list-tags">tag</a></li>
            <li><a href="#registry-tag-manifest">manifest</a></li>
            <li><a href="#registry-tag-delete">delete</a></li>
            <li><a href="#registry-get-token">token</a></li>
        </ul>
    </li>
    <li><a href="#reset">reset</a></li>
    <li><a href="#version">version</a></li>
</ul>
